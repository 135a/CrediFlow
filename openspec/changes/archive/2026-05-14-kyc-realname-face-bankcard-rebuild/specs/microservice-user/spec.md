# microservice-user（Delta，BREAKING）
...
旧 `POST /api/app/user/kyc/step1|step2|step3` MUST 进入 deprecation 期：在 `crediflow.kyc.use-v2=true` 时 MUST 直接返回 `410 Gone` 或业务约定的「请使用 KYC v2」语义；在应急回滚为 `false` 时方可继续工作。
...
- `POST /api/internal/risk/kyc/lookup-by-fingerprint` — 给风控服务按 `id_card_fingerprint` 反查 `userId / kyc_passed` 的轻量接口（MUST NOT 反查明文身份证号）
...
- **THEN** 后门接口 MUST 不被注册到 Spring 上下文；外部请求 MUST 返回 404
