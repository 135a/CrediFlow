# user-kyc-authentication（Delta，BREAKING）

> 本次 change 把原「三步 KYC」从「基础信息 + 二要素 + 简单支付账号」重写为「二要素+闸门 / 人脸实人 / KYC 通过门槛」。
> 旧 step3 的 `paymentMethod/paymentAccount` 字段被新能力 `bankcard-four-elements-binding` 取代；
> 旧 step1 的画像字段（月收入、居住地、职业）从 KYC 主流程剥离，不再阻塞 KYC 通过。
